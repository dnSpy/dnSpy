...
namespace dnSpy.AsmEditor.Compiler {
	struct MDEditorPatcher {
		...
		public MDEditorPatcher(byte[] moduleData, MetadataEditor mdEditor, IAssembly tempAssembly, TypeDef nonNestedEditedTypeOrNull) {
		...
		void PatchTypeRefsToEditedType(TypeDef nonNestedEditedType) {
			...
			using (var stringsStream = md.StringsStream.GetClonedImageStream()) {
				...
				for (uint i = 0; i < table.Rows; i++, offset += rowSize) {
					uint nameOffset = ReadColumn(moduleData, offset, columns[1]);
					if (!StringsStreamNameEquals(stringsStream, nameOffset, nameData))
						continue;
					uint namespaceOffset = ReadColumn(moduleData, offset, columns[2]);
					if (!StringsStreamNameEquals(stringsStream, namespaceOffset, namespaceData))
						continue;

					uint resolutionScopeCodedToken = ReadColumn(moduleData, offset, columns[0]);
		...
		static uint ReadColumn(byte[] data, int offset, ColumnInfo column) {
		...
		void PatchTypeTokenReferences(TypeDef nonNestedEditedType) {
			...
			// Patch the Field table
			{
				...
				for (uint i = 0; i < table.Rows; i++, offset += rowSize) {
					uint sig = ReadColumn(moduleData, offset, column);
