...
namespace dnSpy.AsmEditor.Compiler {
	static class MDPatcherUtils {
		...
		public static uint ReadCompressedUInt32(byte[] data, ref uint offset) {
			...
			if ((b & 0xC0) == 0x80) {
				if (offset >= end)
					throw new IndexOutOfRangeException();
				return (uint)(((b & 0x3F) << 8) | data[offset++]);
			}

			if (offset + 2 >= end)
				throw new IndexOutOfRangeException();
			return (uint)(((b & 0x1F) << 24) | (data[offset++] << 16) |
