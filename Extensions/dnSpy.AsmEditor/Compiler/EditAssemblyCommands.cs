...
namespace dnSpy.AsmEditor.Compiler {
	...
	sealed class EditAssemblyCommand : EditCodeCommandBase {
		...
		sealed class DocumentsCommand : DocumentsContextMenuHandler {
			...
			public override void Execute(AsmEditorContext context) => EditAssemblyCommand.Execute(editCodeVMCreator, methodAnnotations, undoCommandService, appService, context.Nodes);
		}
		...
		sealed class CodeCommand : NodesCodeContextMenuHandler {
			...
			public override void Execute(CodeContext context) => EditAssemblyCommand.Execute(editCodeVMCreator, methodAnnotations, undoCommandService, appService, context.Nodes);
		}
		...
		static void Execute(EditCodeVMCreator editCodeVMCreator, Lazy<IMethodAnnotations> methodAnnotations, Lazy<IUndoCommandService> undoCommandService, IAppService appService, DocumentTreeNodeData[] nodes) {
			...
			undoCommandService.Value.Add(new EditAssemblyCommand(methodAnnotations, modNode, vm.Result));
			vm.Dispose();
		}
