...
namespace dnSpy.Debugger.Exceptions {
	enum ExceptionServiceEventType {
		Restored,
		Removed,
		Added,
	...
	sealed class ExceptionServiceEventArgs : EventArgs {
		public ExceptionServiceEventType EventType { get; }
	...
	interface IExceptionService {
		event EventHandler<ExceptionServiceEventArgs> Changed;
	...
	[Export(typeof(IExceptionService)), Export(typeof(ILoadBeforeDebug))]
	sealed class ExceptionService : IExceptionService, ILoadBeforeDebug {
		...
		public event EventHandler<ExceptionServiceEventArgs> Changed;
		...
		public void RestoreDefaults() {
			...
			Changed?.Invoke(this, new ExceptionServiceEventArgs(ExceptionServiceEventType.Restored));
		}
		...
		public void RemoveExceptions(IEnumerable<ExceptionInfo> infos) {
			...
			if (list.Count != 0) {
				Changed?.Invoke(this, new ExceptionServiceEventArgs(ExceptionServiceEventType.Removed, list));
			}
		}
