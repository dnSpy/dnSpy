...
namespace dnSpy.Debugger.ToolWindows.Locals {
	...
	sealed class CommandsLoader : IAutoLoaded {
		...
		CommandsLoader(IWpfCommandService wpfCommandService, Lazy<VariablesWindowOperations> operations, Lazy<LocalsContent> content) {
			var cmds = wpfCommandService.GetCommands(LocalsContent.VariablesWindowGuid);
