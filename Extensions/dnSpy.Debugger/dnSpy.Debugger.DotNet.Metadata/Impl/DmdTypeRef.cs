...
namespace dnSpy.Debugger.DotNet.Metadata.Impl {
	sealed class DmdTypeRef : DmdTypeBase {
		...
		protected override DmdFieldInfo[] CreateDeclaredFields(DmdType reflectedType) => ResolvedType.DeclaredFields.ToArray();
		protected override DmdMethodBase[] CreateDeclaredMethods(DmdType reflectedType, bool includeConstructors) => ResolvedType.DeclaredMethods.ToArray();
		protected override DmdPropertyInfo[] CreateDeclaredProperties(DmdType reflectedType) => ResolvedType.DeclaredProperties.ToArray();
		protected override DmdEventInfo[] CreateDeclaredEvents(DmdType reflectedType) => ResolvedType.DeclaredEvents.ToArray();
	}
}
