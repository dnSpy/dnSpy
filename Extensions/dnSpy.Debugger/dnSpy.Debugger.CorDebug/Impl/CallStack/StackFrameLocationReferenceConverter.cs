...
namespace dnSpy.Debugger.CorDebug.Impl.CallStack {
	...
	sealed class StackFrameLocationReferenceConverter : ReferenceConverter {
		public override void Convert(ref object reference) {
			switch (reference) {
			case DbgDotNetStackFrameLocationImpl bodyLoc:
				switch (bodyLoc.ILOffsetMapping) {
				...
				case DbgILOffsetMapping.Epilog:
					reference = new DotNetMethodBodyReference(bodyLoc.Module, bodyLoc.Token, bodyLoc.ILOffset);
					break;
				...
				default:
					...
					reference = new DotNetTokenReference(bodyLoc.Module, bodyLoc.Token);
					break;
				}
				break;
			}
