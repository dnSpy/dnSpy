...
namespace dnSpy.AsmEditor {
	...
	sealed class UndoCommandManagerLoader : IPlugin {
		...
		void IPlugin.OnLoaded() {
			...
			MainWindow.Instance.AskUserReloadAssemblyListIfModified = AskUserReloadAssemblyListIfModified;
			MainWindow.Instance.OnShowNewAssemblyList += MainWindow_OnShowNewAssemblyList;
			MainWindow.Instance.SaveTabState += MainWindow_SaveTabState;
		}

		void MainWindow_SaveTabState(object sender, MainWindow.TabStateEventArgs e) {
			var hexTabState = e.TabState as HexTabState;
			if (hexTabState != null && hexTabState.HexBox.Document is AsmEdHexDocument) {
				SaveModule.Saver.SaveAssemblies(new[] { UndoCommandManager.Instance.GetUndoObject((AsmEdHexDocument)hexTabState.HexBox.Document) });
				return;
			}
		}

		void MainWindow_OnShowNewAssemblyList(object sender, EventArgs e) {
		...
		bool AskUserReloadAssemblyListIfModified(string question) {
			...
			var res = MainWindow.Instance.ShowMessageBox(string.Format("{0} {1}", msg, question), System.Windows.MessageBoxButton.YesNo);
