...
namespace dnSpy.Shared.MVVM {
		...
		protected override string Validate() {
			...
			try {
				...
				if (!string.IsNullOrEmpty(ex.Message))
					return ex.Message;
				return string.Format(dnSpy_Shared_Resources.CouldNotConvert, StringValue);
			}
		}
	}
	...
	public sealed class NullableGuidVM : DataFieldVM<Guid?> {
		...
		internal static Guid ParseGuid(string s, out string error) {
			...
			error = dnSpy_Shared_Resources.InvalidGuid;
			return Guid.Empty;
		}
	}
	...
	public sealed class DefaultConverterVM<T> : DataFieldVM<T> {
		...
		protected override string ConvertToValue(out T value) {
			...
			try {
				...
				value = default(T);
				error = string.Format(dnSpy_Shared_Resources.ValueMustBeType, typeof(T).FullName, ex.Message);
			}
			return error;
		}
	}
