...
namespace dnSpy.Files.TreeView {
	sealed class AssemblyFileNode : DnSpyFileNode, IAssemblyFileNode {
		...
		public override Guid Guid {
			get { return new Guid(FileTVConstants.DNSPY_ASSEMBLY_NODE_GUID); }
		}
		...
		protected override void WriteToolTip(ISyntaxHighlightOutput output, ILanguage language) {
			...
			output.WriteLine();
			output.Write(ExeUtils.GetDotNetVersion(DnSpyFile.AssemblyDef.ManifestModule), TextTokenType.EnumField);

			output.WriteLine();
			output.Write(ExeUtils.GetArchString(DnSpyFile.AssemblyDef.ManifestModule), TextTokenType.EnumField);
