...
namespace dnSpy.Contracts.Debugger.DotNet.Breakpoints.Code {
	...
	public abstract class DbgDotNetBreakpointFactory {
		...
		/// <returns></returns>
		public DbgCodeBreakpoint Create(in ModuleId module, uint token, uint offset) =>
		...
		/// <returns></returns>
		public DbgCodeBreakpoint CreateTracepoint(in ModuleId module, uint token, uint offset, string message) =>
		...
		/// <returns></returns>
		public DbgCodeBreakpoint Create(in ModuleId module, uint token, uint offset, DbgCodeBreakpointSettings settings) =>
		...
		/// <param name="offset">IL offset of the breakpoint within the method body</param>
		/// <returns></returns>
		public abstract DbgCodeBreakpoint TryGetBreakpoint(in ModuleId module, uint token, uint offset);
	}
	...
	public readonly struct DbgDotNetBreakpointInfo {
		...
		/// <param name="offset">IL offset of the breakpoint within the method body</param>
		/// <param name="settings">Breakpoint settings</param>
		public DbgDotNetBreakpointInfo(in ModuleId module, uint token, uint offset, DbgCodeBreakpointSettings settings) {
			Module = module;
			Token = token;
			Offset = offset;
			Settings = settings;
