...
namespace dnSpy {
	static class TextOutputUtils {
		...
		public static T WriteNamespace<T>(this T output, string name) where T : ITextOutput {
			if (name == null)
				return output;
			if (name.Length == 0)
				output.Write('-', TextTokenType.Operator);
		...
		public static T WriteModule<T>(this T output, string name) where T : ITextOutput {
