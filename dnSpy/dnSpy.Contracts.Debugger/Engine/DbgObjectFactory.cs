...
namespace dnSpy.Contracts.Debugger.Engine {
	...
	public abstract class DbgObjectFactory {
		...
		/// <returns></returns>
		public DbgEngineThread CreateThread(DbgAppDomain appDomain, string kind, int id, int? managedId, string name) =>
			CreateThread<object>(appDomain, kind, id, managedId, name, null);
		...
		/// <returns></returns>
		public abstract DbgEngineThread CreateThread<T>(DbgAppDomain appDomain, string kind, int id, int? managedId, string name, T data) where T : class;
	}
}
