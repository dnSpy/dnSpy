...
namespace dnSpy.Files.Tabs.Dialogs {
	sealed class FileListColumnConverter : IValueConverter {
		public object Convert(object value, Type targetType, object parameter, CultureInfo culture) {
			var vm = value as FileListVM;
			var s = parameter as string;
			if (vm == null || s == null)
				return null;

			var gen = ColorizedTextElementProvider.Create(vm.Owner.SyntaxHighlight);
			var printer = new FileListPrinter(gen.Output);
			if (StringComparer.OrdinalIgnoreCase.Equals(s, "Name"))
				printer.WriteName(vm);
			else if (StringComparer.OrdinalIgnoreCase.Equals(s, "FileCount"))
				printer.WriteFileCount(vm);
			else
				return null;
