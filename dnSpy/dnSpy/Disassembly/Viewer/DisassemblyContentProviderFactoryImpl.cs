	...
	sealed class DisassemblyContentProviderFactoryImpl : DisassemblyContentProviderFactory {
		...
		public override DisassemblyContentProvider Create(NativeCode code, DisassemblyContentFormatterOptions formatterOptions, ISymbolResolver symbolResolver, string header) {
			...
			case NativeCodeKind.X86_16:
				return new X86DisassemblyContentProviderFactory(x86Deps, 16, formatterOptions, symbolResolver, header, code.Blocks).Create();

			case NativeCodeKind.X86_32:
				return new X86DisassemblyContentProviderFactory(x86Deps, 32, formatterOptions, symbolResolver, header, code.Blocks).Create();

			case NativeCodeKind.X86_64:
				return new X86DisassemblyContentProviderFactory(x86Deps, 64, formatterOptions, symbolResolver, header, code.Blocks).Create();
