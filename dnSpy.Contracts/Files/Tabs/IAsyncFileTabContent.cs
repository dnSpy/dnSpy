...
namespace dnSpy.Contracts.Files.Tabs {
	...
	public interface IAsyncFileTabContent : IFileTabContent {
		/// <summary>
		/// Returns true if <see cref="AsyncWorker(IFileTabUIContext)"/>
		/// should be called
		/// </summary>
		/// <param name="uiContext">UI Context</param>
		/// <returns></returns>
		bool CanStartAsyncWorker(IFileTabUIContext uiContext);

		/// <summary>
		/// Called in the worker thread
		/// </summary>
		/// <param name="uiContext">UI Context</param>
		void AsyncWorker(IFileTabUIContext uiContext);

		/// <summary>
		/// Called in the main UI thread after the worker thread has exited or was interrupted
		/// </summary>
		/// <param name="uiContext">UI Context</param>
		void EndAsyncShow(IFileTabUIContext uiContext);
	}
}
