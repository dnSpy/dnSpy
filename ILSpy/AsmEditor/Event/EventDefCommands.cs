...
namespace ICSharpCode.ILSpy.AsmEditor.Event
{
	sealed class DeleteEventDefCommand : IUndoCommand
	{
		...
		public struct DeleteModelNodes
		{
			...
			public void Delete(ILSpyTreeNode[] nodes)
			{
				...
				for (int i = 0; i < infos.Length; i++) {
					var node = (EventTreeNode)nodes[i];
		...
		{
			this.nodes = new DeletableNodes(eventNodes);
		}
	...
	sealed class CreateEventDefCommand : IUndoCommand
	{
		...
		public void Undo()
		{
			bool b = ownerNode.TypeDefinition.Events.Remove(eventNode.EventDefinition) &&
					ownerNode.Children.Remove(eventNode);
			Debug.Assert(b);
			if (!b)
				throw new InvalidOperationException();
		}
